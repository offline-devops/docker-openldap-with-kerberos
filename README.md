...
## offline-devops/docker-openldap-with-kerberos

![Docker Pulls](https://img.shields.io/docker/pulls/offline-devops/docker-openldap-with-kerberos.svg)
![Docker Stars](https://img.shields.io/docker/stars/offline-devops/docker-openldap-with-kerberos.svg)
![](https://images.microbadger.com/badges/image/offline-devops/docker-openldap-with-kerberos.svg)

Latest release: 1.5.0 - OpenLDAP 2.4.50 -  [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/offline-devops/docker-openldap-with-kerberos/) 
...
- [offline-devops/docker-openldap-with-kerberos](#osixiaopenldap)
	...
	- [Advanced User Guide](#advanced-user-guide)
		- [Extend offlinedevops/offline-devops/openldap-with-kerberos:1.4.0 image](#extend-osixiaopenldap140-image)
...
Run OpenLDAP docker image:
	...
	docker run --name my-openldap-container --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0
...
Do not forget to add the port mapping for both port 389 and 636 if you wish to access the ldap server from another machine.
	...
	docker run -p 389:389 -p 636:636 --name my-openldap-container --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0
...
By default the admin has the password **admin**. All those default settings can be changed at the docker command line, for example:
	...
	--env LDAP_ADMIN_PASSWORD="JonSn0w" --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0
...
argument to entrypoint if you don't want to overwrite them.
      ...
      offlinedevops/offline-devops/openldap-with-kerberos:1.4.0 --copy-service
...
simply mount this directories as a volume to `/var/lib/ldap` and `/etc/ldap/slapd.d`:
	...
	--detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0
...
> [offline-devops/docker-openldap-with-kerberos-backup](https://github.com/osixia/docker-openldap-backup)
...
By default, TLS is already configured and enabled, certificate is created using container hostname (it can be set by docker run --hostname option eg: ldap.example.org).
	...
	docker run --hostname ldap.my-company.com --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0
...
Add --env LDAP_TLS=false to the run command:
	...
	docker run --env LDAP_TLS=false --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0

### Multi master replication
Quick example, with the default config.

	#Create the first ldap server, save the container id in LDAP_CID and get its IP:
	LDAP_CID=$(docker run --hostname ldap.example.org --env LDAP_REPLICATION=true --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0)
	LDAP_IP=$(docker inspect -f "{{ .NetworkSettings.IPAddress }}" $LDAP_CID)

	#Create the second ldap server, save the container id in LDAP2_CID and get its IP:
	LDAP2_CID=$(docker run --hostname ldap2.example.org --env LDAP_REPLICATION=true --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0)
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] offlinedevops/offline-devops/openldap-with-kerberos:1.4.0 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0 --loglevel debug

See all command line options:

	docker run offlinedevops/offline-devops/openldap-with-kerberos:1.4.0 --help
...
- **LDAP_REPLICATION_HOSTS**: list of replication hosts, must contain the current container hostname set by --hostname on docker run command. Defaults to :
  ...
  - ldap://ldap2.example.org
	...
	If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env LDAP_REPLICATION_HOSTS="#PYTHON2BASH:['ldap://ldap.example.org','ldap://ldap2.example.org']" --detach offlinedevops/offline-devops/openldap-with-kerberos:1.4.0
...
passwords using the [Docker secrets](https://docs.docker.com/engine/swarm/secrets/) mechanism. For example:
	...
	docker run --env LDAP_ORGANISATION="My company" --env LDAP_DOMAIN="my-company.com" \
	--env LDAP_ADMIN_PASSWORD_FILE=/run/secrets/authentication_admin_pw --detach offlinedevops/docker-openldap-with-kerberos:1.2.4
...
### Extend offlinedevops/offline-devops/openldap-with-kerberos:1.4.0 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

	FROM offlinedevops/offline-devops/openldap-with-kerberos:1.4.0
...
Adapt Makefile, set your image NAME and VERSION, for example:
	...
	NAME = offline-devops/docker-openldap-with-kerberos
	VERSION = 1.1.9
